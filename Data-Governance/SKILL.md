...
│   └── dq_ddl_guide.md                    # 数据质量检查：DDL 编写规范参考指南
└── examples/                   # 输入示例文件
...
| DDL + 数仓连接配置（无 SQL、无标准字段库） | 表级检查 + 字段级检查 + 数据质量 |
| DDL + SQL + 数仓连接配置 + 标准字段库（全量输入） | 全部六个子 skill |
| 只有数仓连接配置（无 DDL、无 SQL） | 数据质量检查无法执行（需要 DDL 才能识别表结构），提示用户补充 DDL |
| 无法识别内容类型 | 全量执行（缺标准字段库则跳过标准绑定；缺连接配置则跳过数据质量） |
